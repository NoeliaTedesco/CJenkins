...
public class D_CambiarContrasenia_LongitudStep extends BaseStep {
	...
	public static void Run(String testName) {
		...
		try {
			...
			nombrePaso = "4_Se verifica la validación de la longitud de la contraseña - Nueva Contraseña";
			CurrentPage = (new D_CambiarContraseniaPage().GetInstance(D_CambiarContraseniaPage.class));
			CurrentPage.As(D_CambiarContraseniaPage.class).ingresarNuevaContrasenia(pass.getContraseniaLongitud());
			CurrentPage.As(D_CambiarContraseniaPage.class).ingresarReingresarContrasenia(pass.getContraseniaLongitud());
			CurrentPage.As(D_CambiarContraseniaPage.class).clickCampoNuevaContrasenia();
			
			Assert.assertEquals(CurrentPage.As(D_CambiarContraseniaPage.class).obtenerErrorLabelNuevaContrasenia(),"El campo es muy corto");
			log.Log.SuccessStep(nombrePaso);
			
			nombrePaso = "5_Se verifica la validación de la longitud de la contraseña - Reingresar Contraseña";
			Assert.assertEquals(CurrentPage.As(D_CambiarContraseniaPage.class).obtenerErrorLabelReingresarContrasenia(), "El campo es muy corto");
			log.Log.SuccessStep(nombrePaso);
