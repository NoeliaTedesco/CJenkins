...
public class D_EnvioInvitacion_GenerarInvitacionSinMedicoStep extends BaseStep {
	...
	public static void Run(String testName) {
		...
		try {
			...
			nombrePaso = "2_ Se ingresa al modulo invitaciones";
			...
			nombrePaso = "6_Se Envia formulario";
			Assert.assertTrue(CurrentPage.As(D_Envio_InvitacionesPage.class).clicBotonEnviar());
			log.Log.SuccessStep(nombrePaso);
			
			nombrePaso = "7_Se verifica mensaje error al no existir medico en guardia";
			Assert.assertTrue(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerMensajeAlertaLogin("No hay ningún médico de guardia para la especialidad seleccionada"));
			log.Log.SuccessStep(nombrePaso);
