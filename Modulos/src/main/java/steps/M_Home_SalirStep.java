...
public class M_Home_SalirStep extends BaseStep {
	...
	public static void Run(String testName) {
		...
		try {
			...
			nombrePaso = "2_Se verifica la visualización del botón Salir";
			...
			nombrePaso = "3_Se hace clic en el botón salir";
			CurrentPage.As(M_HomePage.class).clicBtnSalir();
			StepHelper.takeScreenShot(testName);
			log.Log.SuccessStep(nombrePaso);
			
			nombrePaso = "4_Se verifica la visualización del campo email del login";
			CurrentPage = (new M_LoginPage().GetInstance(M_LoginPage.class));
			Assert.assertTrue(CurrentPage.As(M_LoginPage.class).esVisibleCampoEmail());
			log.Log.SuccessStep(nombrePaso);
