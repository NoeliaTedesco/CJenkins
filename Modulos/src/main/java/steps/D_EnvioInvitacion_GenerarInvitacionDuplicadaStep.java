...
public class D_EnvioInvitacion_GenerarInvitacionDuplicadaStep extends BaseStep {
	...
	public static void Run(String testName) {
		...
		try {
			...
			nombrePaso = "6_Se Envia formulario con datos de nro Socio con invitación ya existente";
			Assert.assertTrue(CurrentPage.As(D_Envio_InvitacionesPage.class).clicBotonEnviar());
			log.Log.SuccessStep(nombrePaso);
			
			nombrePaso = "7_Se verifica mensaje de invitación duplicada";
			Assert.assertTrue(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerMensajeAlertaLogin("El paciente ya tiene una invitación pendiente"));
			log.Log.SuccessStep(nombrePaso);
