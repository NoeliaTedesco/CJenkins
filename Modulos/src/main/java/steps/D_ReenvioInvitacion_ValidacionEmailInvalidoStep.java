	...
	public static void Run(String testName) {
		...
		try {
			...
			if (inv.getEmail().contains("+")) {
				nombrePaso = "5_Se verifica el mensaje de error del campo: email - no válido - posee caracteres especiales";
				Assert.assertEquals(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerErrorReenvioLabelEmail(),"El campo debe ser un email válido");	
				log.Log.SuccessStep(nombrePaso);	
			}
			
			if (!inv.getEmail().contains("@") && !inv.getEmail().isEmpty()) {
				nombrePaso = "5_Se verifica el mensaje de error del campo: email - no válido - no contiene @";
				Assert.assertEquals(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerErrorReenvioLabelEmail(),"El campo debe ser un email válido");	
				log.Log.SuccessStep(nombrePaso);	
			}
			
			if (inv.getEmail().contains("@..@")) {
				nombrePaso = "5_Se verifica el mensaje de error del campo: email - no válido - contiene @..@";
				Assert.assertEquals(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerErrorReenvioLabelEmail(),"El campo debe ser un email válido");	
				log.Log.SuccessStep(nombrePaso);	
			}
			
			if (inv.getEmail().contains("123(abc)@gmail")) {
				nombrePaso = "5_Se verifica el mensaje de error del campo: email - no válido - caracteres no permitidos";
			...
			if (inv.getEmail().contains("@com.")) {
				nombrePaso = "5_Se verifica el mensaje de error del campo: email - no válido - punto al final";
				Assert.assertEquals(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerErrorReenvioLabelEmail(),"El campo debe ser un email válido");	
				log.Log.SuccessStep(nombrePaso);	
			}
			
			if (inv.getEmail().contains("12345678")) {
				nombrePaso = "5_Se verifica el mensaje de error del campo: email - no válido - solo valores numéricos";
				Assert.assertEquals(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerErrorReenvioLabelEmail(),"El campo debe ser un email válido");	
				log.Log.SuccessStep(nombrePaso);	
			}
