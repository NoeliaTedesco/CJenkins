...
public class D_EnvioInvitacion_ValidacionNroCelularStep extends BaseStep {
	...
	public static void Run(String testName) {
		...
		try {
			...
			if (inv.getNroCelular().length() < 10) {
				nombrePaso = "6_Se verifica texto error de campos celular - Longitud menor a 10";
				Assert.assertEquals(CurrentPage.As(D_Envio_InvitacionesPage.class).obtenerErrorLabelTelefono(),"El campo es muy corto");	
				log.Log.SuccessStep(nombrePaso);	
			}
			
			if (inv.getNroCelular().length() > 10) {
				nombrePaso = "6_Se verifica maximo de ingreso de digitos en el campo Nro Telefono - Longitud mayor a 10";
			...
			if (inv.getNroCelular().equals("nrocelular")) {
				nombrePaso = "6_Se verifica el no ingreso de letras en el campo Nro Telefono";
