...
public class D_SeccionInvitacion_MostrarActivas_TildarStep extends BaseStep {
	...
	public static void Run(String testName) {
		...
		try {
			...
			nombrePaso = "3_Se verifica la visualizacion del boton enviar invitacion";
			CurrentPage = (new D_Envio_InvitacionesPage().GetInstance(D_Envio_InvitacionesPage.class));
			Assert.assertTrue(CurrentPage.As(D_Envio_InvitacionesPage.class).estaHabilitadoBtnInvitaciones());
			log.Log.SuccessStep(nombrePaso);
			...
			nombrePaso = "6_Se verifica que se muestre el boton Mostrar solo activas";
			CurrentPage = (new D_Envio_InvitacionesPage().GetInstance(D_Envio_InvitacionesPage.class));
			Assert.assertTrue(CurrentPage.As(D_Envio_InvitacionesPage.class).esVisibleCheckGrillaInvitaciones());
			log.Log.SuccessStep(nombrePaso);
			
			nombrePaso = "7_Se verifica que se muestre la invitacion activa";
